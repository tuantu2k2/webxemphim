<template>
    <main class="page__main">
        <article class="section__film-grid" v-if="slugs">
            <h1 class="section__title section__title--primary" > <span class="fas fa-film title__icon--film"></span> Tìm kiếm cho từ khóa "{{keySearch}}""
...
export default {
    ...
    methods: {
        async retrieveFilms() {
            try {
                this.keySearch = this.id.replaceAll("-"," ")
        ...
        checkSearchValue(category){ 
                    // chuyen slug ve string chuan de dễ so sánh
                    if(category.includes(this.id)) return true
        ...
        getInforFilm(slug) {
            axios.get(`https://ophim1.com/phim/${slug}`)
                .then(res => {
                    if(this.checkSearchValue(res.data.movie.slug))
